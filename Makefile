...
test-race:
	CGO_ENABLED=1 go test -count=1 -timeout=40m -race ./...

test-oracle:
	go test -count=1 -timeout=40m -tags oracle ./...

cover:
	go test -count=1 -timeout=40m -coverprofile=cover.out -covermode=atomic ./...
